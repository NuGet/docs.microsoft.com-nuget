...
| RestoreUseSkipNonexistentTargets | Determines if MSBuild should be called with `SkipNonexistentTargets`. When not set, defaults to `true`.| Available since 4.8.1. Possible values *true* (default), *false*. |
