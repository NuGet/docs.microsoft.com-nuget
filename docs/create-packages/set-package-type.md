...
- `McpServer` type packages contain MCP servers. This package type is always accompanied by the `DotnetTool` package type, because a local MCP server is distributed as a .NET tool. For information on how to build an MCP server, see [the quickstart guide](/dotnet/ai/quickstarts/build-mcp-server). 
