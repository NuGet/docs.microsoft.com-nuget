...
- [`IVsFrameworkParser`](#ivsframeworkparser-interface) An interface for dealing with the conversion between strings and `FrameworkName` TODO NK - add link (4.0+)
- [`IVsFrameworkParser`](#ivsframeworkparser2-interface) An interface to parse .NET Framework strings. See <a href="http://aka.ms/NuGet-IVsFrameworkParser">http://aka.ms/NuGet-IVsFrameworkParser</a>. (5.8+)
...
- [`IVsPathContextProvider`](#ivspathcontextprovider-interface) A factory to initialize <see cref="IVsPathContext"/> instances. (4.0+)
- [`IVsPathContextProvider2`](#ivspathcontextprovider2-interface) A factory to initialize <see cref="IVsPathContext2"/> instances. (5.0+)
