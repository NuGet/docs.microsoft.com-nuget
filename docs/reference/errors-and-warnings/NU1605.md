...
You can also try to manually update the packages that are giving you the downgrade error.   
In this example, since `Microsoft.NETCore.App` is needing to be updated to 2.1.8, in visual studio, we will go to NuGet manager and update this package.   
To do that, **right-click** on your project, and go to **Manage NuGet Packages...** option. Within the **Browse** tab, search the `Microsoft.NETCore.App` package and on the right-hand panel, update the version to the specified version needed. In our case, we would want to update `2.1.0` to `2.1.8`. After it has finished installing, try to rebuild your project. 
