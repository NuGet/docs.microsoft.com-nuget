...
> [!NOTE]
> While NU1605 is considered a warning by the NuGet tooling, the .NET SDK opts into treating this warning as an error through `WarningsAsErrors`. You may choose to opt out of this (TODO NK - Add an example.)
> You may have upgraded this warning to error yourself by setting `TreatWarningsAsErrors` to `true`.