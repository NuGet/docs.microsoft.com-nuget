...
* If you are using Visual Studio 2019 or later, NuGet has the ability to re-use artifacts from the previous restore for unloaded projects.
In order for this scenario to work, you need to ensure that all projects currently in the solution have been restored from the command line prior to loading them in Visual Studio.
...
- If you intentionally exclude the project from the solution, run a command-line restore for the solution once before relying on Visual Studio's built-in restore.
Visual Studio can then re-use the restore artifacts for the excluded project.