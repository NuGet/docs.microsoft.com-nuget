...
When a referenced project brings in a `FrameworkReference`, add the same `FrameworkReference` directly to the current project if you want pruning to account for it.

In this example, `Library.csproj` references `Microsoft.AspNetCore.App`, and `Consumer.csproj` references `Library.csproj`.
Without a direct `FrameworkReference` in `Consumer.csproj`, `Microsoft.Extensions.Caching.Memory` isn't considered prunable in the consumer project.
...
NuGet can then determine that `Microsoft.Extensions.Caching.Memory` is provided by the shared framework and raises NU1510 for the unnecessary direct package reference.
...
If the package is only prunable because a referenced project brings in a framework transitively, add the matching `FrameworkReference` directly to the current project first.
