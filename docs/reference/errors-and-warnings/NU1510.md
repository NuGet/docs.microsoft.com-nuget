...
The list of packages used for pruning is determined by the direct `FrameworkReference` items of the current project.
`FrameworkReference` items are transitive.
However, when the current project inherits a framework reference through a `ProjectReference`, pruning doesn't use that transitive framework reference to remove packages.
Note that this package will be removed by build-time conflict resolution.
...
Add the matching `FrameworkReference` directly to the current project.
Then remove the unnecessary `PackageReference`.
