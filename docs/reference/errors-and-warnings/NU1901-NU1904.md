...
For projects targetting the .NET 10 framework, the default setting for `NuGetAuditMode` is `all`.
This means that NuGet will report on transitive packages with known vulnerabilities.
