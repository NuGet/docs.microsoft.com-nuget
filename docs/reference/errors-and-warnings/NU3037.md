...
When NuGet’s signature validation mode is set to require, a package with an expired package signature is blocked from installation, and NU3037 is elevated from a warning to an error. Require mode requires non-expired package signatures.
