...
| NUGET_SCRATCH | Path to the temporary folder where NuGet stores temporary files during its various operations, including the lock files used to synchronize concurrent NuGet processes. | Available in NuGet 6.2 and higher. Specified as an absolute path. When not set, the default depends on the operating system. On Windows and Mac, the default `NuGetScratch` folder is in the per-user temporary directory. On Linux, the default is `/tmp/NuGetScratch<username>`. |
