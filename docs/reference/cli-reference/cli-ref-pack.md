...
| Deterministic | Specify if the command should create a deterministic package. Multiple invocations of the pack command will generate the exact same package. The output of pack is not affected by the ambient state of the machine. Specifically, package internal psmdcp will be generated based on the package files, and the zip entries will be timestamped as 1/1/1980. |
