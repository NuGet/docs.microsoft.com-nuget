...
- **`-ApiKey`**
  ...
  The API key for the target repository. If not present, the one specified in the config file is used. If not present in the config file, NuGet 7.6+ checks the `NUGET_API_KEY` environment variable.
...
- **`-SymbolApiKey`**
  ...
  *(3.5+)* Specifies the API key for the URL specified in `-SymbolSource`. If not present, NuGet 7.6+ checks the `NUGET_SYMBOL_API_KEY` environment variable.
