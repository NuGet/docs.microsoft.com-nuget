...
| maxHttpRequestsPerSource | Controls the maximum number of parallel requests sent from NuGet to every package source for package dependency resolution and downloads. The default value on `.NET Core` is `Int32.MaxValue` which is derived from `HttpClientHandler.MaxConnectionsPerServer` property. This setting has no impact on `.NET Core` for `Mac OS` because the throttling limit is set to `16` to avoid too many open files error. `NuGet client tools` that runs on `.NET Framework` such as `Visual Studio` and `nuget.exe` sets `ServicePointManager.DefaultConnectionLimit` property to `64` on `Windows` and to `1` on `Mono` to control maximum number of concurrent connections allowed for every package source. Hence configuring `maxHttpRequestsPerSource` property to a value less than `64` could impact performance of `Visual Studio` and `nuget.exe` on `Windows`. `Packages.config` style projects use `maxDegreeOfParallelism` which is set to `Environment.ProcessorCount` by default for package dependency resolution and downloads. Hence configuring `maxHttpRequestsPerSource` property to a value less than `Environment.ProcessorCoun` could impact performance of `Packages.config` style projects. |
