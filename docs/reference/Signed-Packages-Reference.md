...
> [!Important]
> Package signing is currently supported only when using nuget.exe on Windows. [Verification of signed packages is currently supported only when using nuget.exe](cli-reference.md/cli-ref-verify) or Visual Studio on Windows.
