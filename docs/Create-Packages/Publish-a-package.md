...
Packages pushed to NuGet.org undergo several validations. When the package has passed all validation checks, it might take a while for it to be indexed and appear in serach resutlts. Once indexing is complete, you'll receive an email confirming that the package was sucessfully published. If the package fails a validation check, the package details page will update to display the associated error and you'll also receive an email notifying you about it.
