...
If you produce Visual Studio templates, you can use NuGet to [preinstall packages](/ndocs/visual-studio-extensibility/visual-studio-templates) as part of the template.  Until now, this feature had a limitation that all of the packages needed to come from the same source.  With NuGet 2.2.1 though, you can have packages installed from multiple repositories (within the template, a VSIX, or a folder on disk defined in the registry).
