...
- You make changes to a published package, but you forget to update the version number before you try to publish it again. In this case, the package already exists on the host.
...
To address this problem, change the package identifier to be unique, rebuild the project, re-create the *.nupkg* file, and retry the `push` command.
