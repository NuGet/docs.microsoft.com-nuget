...
> * There is a limitation when using the `DotNetCoreCLI@2` Azure Pipelines task which requires `feed-` prefixes in your source mapping configuration, it is recommended to use `NuGetAuthenticate` for authentication needs instead, see [microsoft/azure-pipelines-tasks#15542](https://github.com/microsoft/azure-pipelines-tasks/issues/15542).
