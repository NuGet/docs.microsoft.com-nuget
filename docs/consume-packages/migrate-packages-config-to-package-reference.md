...
In addition, there are some differences in how PackageReferences work compared to packages.config. For example - [constraining upgrade versions](/reinstalling-and-updating-packages#constraining-upgrade-versions) is not supprted by PackageReference but add support for [Floating Versions](/consume-packages/package-references-in-project-files#floating-versions)
