...
When a project uses [Central Package Management (CPM)](Central-Package-Management.md), a `<PackageVersion />` item must not be defined in `Directory.Packages.props` for an implicitly defined package and instead must come from the `PackageReference` item itself.
