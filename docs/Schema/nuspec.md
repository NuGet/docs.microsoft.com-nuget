...
**iconUrl** | A URL for a 64x64 image with transparency background to use as the icon for the package in UI display. Note: this should be the image URL and not a webpage containing the image. E.g. if you want to put an image URL from GitHub, make sure it is the raw file URL (like `https://github.com/<username\>/<repsitory>/raw/<branch>/<logo.png>`)
