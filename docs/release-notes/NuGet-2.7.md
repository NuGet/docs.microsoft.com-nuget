...
14. [Michael Friis](https://www.codeplex.com/site/users/view/friism) ([@friism](https://twitter.com/friism))
     - [#3278](https://blog.nuget.org/workitem/3278) - Fixes NuGet.Build targets file to so that it works correctly on MonoDevelop
...
There were two known issues with nuget.exe restore with the initial 2.7 release, but they were fixed on 9/6/2013 with an update to the [NuGet.CommandLine package](http://www.nuget.org/packages/NuGet.CommandLine/).  This update is also available on the [NuGet 2.7 download page](https://blog.nuget.org/releases/view/107605) on CodePlex.  Running `nuget.exe update -self` will update to the latest release.

The fixed were:

1. [New package restore doesn't work on Mono when using SLN file](https://blog.nuget.org/workitem/3596)
1. [New package restore doesn't work with Wix projects](https://blog.nuget.org/workitem/3598)

There is also a known issue with the new package restore workflow whereby [Automatic Package Restore does not work for projects under a solution folder](https://blog.nuget.org/workitem/3625). This issue was fixed in NuGet 2.7.1.
...
To download the NuGet Package Manager (for Visual Studio 2010 Express for Windows Phone), visit the [NuGet 2.7 Downloads](https://blog.nuget.org/releases/view/107605) page.

### Bug Fixes

In addition to these features, this release of NuGet also includes many other bug fixes. There were 97 total issues addressed in the release. For a full list of work items fixed in NuGet 2.7, please view the [NuGet Issue Tracker for this release](https://blog.nuget.org/workitem/list/advanced?release=NuGet%202.7&status=all).
